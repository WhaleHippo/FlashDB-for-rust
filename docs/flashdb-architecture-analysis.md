...
대상 워크스페이스: `~/Desktop/FlashDB-for-embassy`
목적: STM32 + Embassy 환경에서 동작하는 Rust 기반 FlashDB 재구현의 선행 분석 기록

## 1. 분석 목표

이 문서는 C로 작성된 FlashDB 원본 프로젝트의 전체 구조를 Rust/Embassy 관점에서 재구현하기 위해 정리한 아키텍처 분석 보고서다.
초점은 다음 네 가지다.

1. 원본 프로젝트의 디렉터리/모듈 구조 파악
2. KVDB/TSDB의 온플래시 데이터 레이아웃과 상태 전이 파악
3. 초기화, 복구, GC, 순회, 포팅 계층의 핵심 동작 파악
4. Rust + Embassy 포팅 시 유지해야 할 개념과 재설계가 필요한 지점 분리

이 문서는 이후 `FlashDB-for-embassy` 구현 시 설계 기준 문서로 계속 확장하는 것을 전제로 작성한다.
...
Embassy/NorFlash 포팅에서도 반드시 같은 추상화가 필요하다.
...
Rust/Embassy 최종 목표에서는 필수는 아니지만, host-side 시뮬레이터 또는 integration test용 백엔드로 매우 유용하다.
...
Rust/Embassy로 가면 이 값은 `embedded_storage::nor_flash::NorFlash::WRITE_SIZE`와 대응된다.
...
Embassy 포팅에서 FAL 자체를 재현할 필요는 없다.
...
즉 FlashDB-for-embassy는 “FAL replacement”가 아니라 “NorFlash region adapter”를 두는 방향이 적절하다.
...
## 10. Rust + Embassy 포팅 시 그대로 가져가야 할 핵심 설계
...
## 11. Rust + Embassy 포팅 시 재설계/개선할 부분

결론부터 말하면 재설계/개선할 부분이 분명히 존재한다.
다만 전부 바꾸는 것이 아니라, FlashDB의 본질을 유지하면서 “Rust/Embassy 환경에 맞게 더 안전하고 검증 가능하게” 바꾸는 쪽이 좋다.
...
Embassy 대상 제품이 장기 운용 장비라면 개선 여지가 있다.
...
## 12. FlashDB-for-embassy용 권장 모듈 분해안
...
## 15. Embassy 관점에서의 직접 대응표

원본 개념 -> Rust/Embassy 대응안
...
- `lock/unlock` callback -> `embassy_sync::mutex::Mutex` 또는 상위 동기화 정책
...
따라서 Rust/Embassy 버전도 단순 API 모방이 아니라, 이 불변 조건들을 재현해야 FlashDB다운 구현이 된다.
...
오히려 FlashDB를 Rust/Embassy로 가져갈 때는 KV/TS record 자체보다 Blob abstraction을 어떻게 재정의하느냐가 API 품질과 메모리 사용성에 큰 영향을 준다.
...
이 설계는 C에서는 가볍고 실용적이지만, Rust/Embassy에서는 몇 가지 개선 포인트가 있다.
...
하지만 Rust/Embassy에서는 그 상태로 옮기기보다 아래처럼 재정의하는 것이 더 좋다.
...
1. FlashDB-for-embassy crate의 공통 레이아웃/스토리지 추상화부터 설계
2. KVDB MVP부터 먼저 구현
